{ 
    ...
    def isBlob(id: Int) = if (blake2b256(OUTPUTS(id).propositionBytes) == blobScriptHash ) {
        if (OUTPUTS(id).tokens.size >= 1) {
            OUTPUTS(id).tokens(0)._1 == GameTokenNFTId              &&
            OUTPUTS(id).tokens(0)._2 >= 1                           &&
            OUTPUTS(id).R4[Coll[Byte]].isDefined                    &&
            OUTPUTS(id).R5[Coll[Long]].isDefined                    &&
            OUTPUTS(id).R6[SigmaProp].isDefined                     &&
            OUTPUTS(id).R7[Long].isDefined                          &&
            OUTPUTS(id).R8[Long].isDefined                          &&
            OUTPUTS(id).R9[Long].isDefined                          &&
    ...
    // KILL BLOB
    val validKill = if (!validBlob0 && OUTPUTS.size > 2) {
    ...
    // ADD/WIDTHDRAW FUNDS
    val validAddWidthdrawFund = if (validBlob0 && blobStateIn == 0 && blobStateValueIn == 0) {
        if (OUTPUTS.size > 2) {
    ...
    // SET STATUS
    val validSetStatus = if (validBlob0 && blobStateIn == 0 && blobStateValueIn == 0) {
        OUTPUTS(0).value == blobValueIn                               &&
        OUTPUTS(0).tokens(0)._2 == 2                                  &&
        OUTPUTS(0).R4[Coll[Byte]].get == blobDescIn                   &&
        isBlobR5Replicated(0)                                         &&
        OUTPUTS(0).R6[SigmaProp].get == ownerPKin                     &&
        OUTPUTS(0).R8[Long].get >= blobMinValue                       &&
        OUTPUTS(0).R9[Long].get == blobUniqueId                       &&
        (
    ...
    // SELL BLOB
    val validSell = if (validBlob0 && (blobStateIn == 2) && !validUnsetStatus) {
        if (OUTPUTS.size > 3) {
        ...
        if (OUTPUTS.size >= 4) {
            if (blake2b256(OUTPUTS(2).propositionBytes) == gameScriptHash) {
                (
                    ...
                    OUTPUTS(0).R8[Long].get == blobStateValueIn                   &&
                    OUTPUTS(0).R9[Long].get == blobUniqueId                       &&
                    OUTPUTS(0).tokens(0)._2 == 1                                  &&
                    // blob in first position for the figth
                    OUTPUTS(2).value >= 2 * blobStateValueIn - txFee              &&
                    OUTPUTS(2).R4[SigmaProp].get == ownerPKin                     &&
                    OUTPUTS(2).R5[Long].get == blobUniqueId                       &&
                    OUTPUTS(2).R6[Coll[Long]].get(0) == blobAttLevelIn            &&
                    OUTPUTS(2).R6[Coll[Long]].get(1) == blobDefLevelIn            &&
                    OUTPUTS(2).R6[Coll[Long]].get(2) == blobNumGameIn             &&
                    OUTPUTS(2).R6[Coll[Long]].get(3) == blobNumWinIn
                    )
                ||
                    ( // blob replicated 1
                    ...
                    OUTPUTS(1).R8[Long].get == blobStateValueIn                   &&
                    OUTPUTS(1).R9[Long].get == blobUniqueId                       &&
                    OUTPUTS(1).tokens(0)._2 == 1                                  &&
                    // blob in second position for the figth
                    OUTPUTS(2).value >= 2 * blobStateValueIn - txFee              &&
                    OUTPUTS(2).R7[SigmaProp].get == ownerPKin                     &&
                    OUTPUTS(2).R8[Long].get == blobUniqueId                       &&
                    OUTPUTS(2).R9[Coll[Long]].get(0) == blobAttLevelIn            &&
                    OUTPUTS(2).R9[Coll[Long]].get(1) == blobDefLevelIn            &&
                    OUTPUTS(2).R9[Coll[Long]].get(2) == blobNumGameIn             &&
                    OUTPUTS(2).R9[Coll[Long]].get(3) == blobNumWinIn
                    )
                )
            } else {
                false
